        ...
        if isCallMth(func_name) and SegName(functionAddr) == ".plt":
            ...
            for ss in ssCallMth:
                ...
                if GetMnem(ss[1]).lower() in jumpInstrction:
                    ...
                    if len(stringConstants) == 0:
                        #get all String in .rodata, 判断引用中是否在calllerName方法中且在当前指令之前，则加入stringConstants
                        for i in Strings():
                            if SegName(i.ea) == ".rodata":
                                xrefs = XrefsTo(i.ea, 0) 
                                for xref in xrefs:
                                    #print "xref:"+ hex(xref.frm)
                                    if function_head < xref.frm and xref.frm < ss[1]:
                                        print "str:"+str(i)
                                        stringConstants.append(str(i))
...
def writeFile(source_mth, sink_mth, androidLogCallMth, runtime):
    ...
    f.write("runtime: " + str(int(runtime)))
    f.write("\n")
