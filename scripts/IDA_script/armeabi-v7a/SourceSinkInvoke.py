...
def recursive(address, frm, nativeMth):
    ...
    for add in refList:
        if isCode(GetFlags(add.frm)):
            ...
            recursive(function_head, add.frm, nativeMth)

def recursiveSource(address, frm, nativeMth):

    func_name = GetFunctionName(address)
    if GetFunctionFlags(address)  & FUNC_FRAME: #是函数入口（C++实现中声明的函数）
        nativeMth.add( (func_name,frm) )
        return
    xrefs = XrefsTo(address, 0)
    refList = list(xrefs)
    ...
    for add in refList:
        if isCode(GetFlags(add.frm)):
            function_head = GetFunctionAttr(add.frm, idc.FUNCATTR_START)
            print "function_head: "+hex(function_head)
            recursiveSource(function_head, add.frm, nativeMth)
def genFunc2String():
    ...
    for functionAddr in Functions():
        ...
        if isCallMth(func_name) and SegName(functionAddr) == ".plt":
            print func_name
            ssCallMth = set()
            recursiveSource(functionAddr,functionAddr, ssCallMth)#找到export函数,以及blx
            ...
            for ss in ssCallMth:
                ...
                if GetMnem(ss[1]).lower() in jumpInstrction:
                    ...
                    while addr > function_head:
                        ...
                        #print hex(addr)
                        ref_address =  GetOperandValue(addr, 1)
        ...
        if func_name == "__android_log_print" and SegName(functionAddr) == ".plt":
            ...
            recursive(functionAddr,functionAddr, androidLogCallMth)
            # print len(nativeMth)
...
if __name__ == '__main__':
    ...
    for n in androidLogCallMth:
            print "__android_log_print method called by method: "+n[0]
    #写结果到文件
