...
public class API {
	...
	public void dumpSpecSourcesAndSinks() {
		try {
			...
			for (SootMethod source : srcsMapping.keySet()) {
				output.write(source + " -> " );
				for (InfoKind kind : srcsMapping.get(source)) {
					output.write(kind +",");
				}
				output.write("\n");
			}
			
			output.write("\nSink Methods Mappings:\n");
			for (SootMethod sink : sinksMapping.keySet()) {
				output.write(sink + " -> " );
				for (InfoKind kind : sinksMapping.get(sink)) {
					output.write(kind +",");
				}
				output.write("\n");
			}
...
//				logger.info("Found verified method: {}", method);
	...
	private void write(String fileName, SootMethod method,String infoKind, String ss) {
		String filePath = Config.v().getApacHome() + File.separator + fileName;
		String bytecodeSignature = method.getBytecodeSignature();

		String[] temp = bytecodeSignature.split(": ");
		String thisT = temp[0].substring(1);
		String methodName = temp[1].substring(0, temp[1].indexOf("("));

        StringBuilder rightThis = new StringBuilder();
        getDescriptor(thisT,rightThis);

		String rightSig = rightThis.toString() + "." + methodName + ":" + temp[1].substring(temp[1].indexOf("("),temp[1].length()-1);
        ...
        try{
            ...
            output.write(rightSig + " "+ infoKind + " -> " + ss);
            output.write("\r\n");//换行
            output.flush();
            output.close();
        }catch (Exception e){
    ...
    private static void getDescriptor( String type, final StringBuilder buf) {
	    String str = type;
        while (true) {
            if (str.contains("[")) {
                buf.append('[');
	...
	public void addSourceInfoKind(SootMethod sootMethod, String kind, boolean sensitive) {
		...
		logger.info("Adding new source infokind category for {} as {}", sootMethod, kind);
	}

	public void addSrcInfoKind(SootMethod sootMethod, String kind, boolean sensitive) {
		if (!srcsMapping.containsKey(sootMethod)) {
			srcsMapping.put(sootMethod, new HashSet<InfoKind>());
		}
		srcsMapping.get(sootMethod).add(InfoKind.getInfoKind(kind, sensitive));
		...
		sourcesThatTaintArgs.add(sootMethod);
		logger.info("Noting that source taints args: {}", sootMethod);
	}

	public void addSinkInfoKind(SootMethod sootMethod,String infoKind, boolean sensitive){
		...
		logger.info("Adding new sink infokind category for {} as {}", sootMethod, infoKind);
	}
	...
	private void addSinkTag(SootMethod sootMethod, AnnotationTag at) {
		...
		for (AnnotationElem ae : ((AnnotationArrayElem)elem).getValues()) {
			...
			infoKind = ((AnnotationEnumElem)ae).getConstantName();
//			logger.info("specific sink infoKind: {}",infoKind);
...
//        write("DSSink.txt",sootMethod, infoKind, "_SINK_");
	}
...
//        write("DSSource.txt",sootMethod, infoKind, "_SOURCE_");
	...
	public SootMethodList getAllSysMethods() {
		return all_sys_methods;
	}
